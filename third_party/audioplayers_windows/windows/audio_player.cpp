...
AudioPlayer::AudioPlayer(
    ...
    std::shared_ptr<EventStreamBridge> eventBridge)
    : _playerId(playerId),
      _methodChannel(methodChannel),
      _eventBridge(std::move(eventBridge)) {
...
void AudioPlayer::OnError(const std::string& code,
  ...
  if (this->_eventBridge) {
    this->_eventBridge->Error(code, message, details);
  }
}
...
void AudioPlayer::OnPrepared(bool isPrepared) {
  if (this->_eventBridge) {
    this->_eventBridge->Success(std::make_unique<flutter::EncodableValue>(
...
void AudioPlayer::FlushPendingEvents() {
  if (this->_eventBridge) {
    this->_eventBridge->DrainPending();
  }
}
